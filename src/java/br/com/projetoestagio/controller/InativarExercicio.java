 ...
 */
@WebServlet(name = "InativarExercicio", urlPatterns = {"/InativarExercicio"})
public class InativarExercicio extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (dao.inativar(exercicio)) {
                mensagem = "Exercicio inativado com sucesso!";
            } else {
                mensagem = "Problemas ao inativar Exercicio!";
            }
            request.setAttribute("mensagem", mensagem);
            request.getRequestDispatcher("ListarExercicio").forward(request, response);
        } catch (Exception ex) {
            System.out.println("Problemas no Servlet ao inativar Exercicio! Erro: " + ex.getMessage());
            ex.printStackTrace();
        }
