 ...
 * @author Leticia
 */
public class CarregarCardapio extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            request.setAttribute("cardapio", dao.carregar(idCardapio));

            GenericDAO daoPaciente = new PacienteDAOImpl();
            request.setAttribute("lista", daoPaciente.listar());
            request.getRequestDispatcher("alterarcardapio.jsp").forward(request, response);
        } catch (Exception ex) {
            System.out.println("Erro no Servlet CarregarCardapio" + ex.getMessage());
            ex.printStackTrace();
        }
