...
public class CarregarAlimento extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            alimento = (Alimento) dao.carregar(idAlimento);
            request.setAttribute("alimento", alimento);
            request.getRequestDispatcher("nutricionista/alteraralimento.jsp").forward(request, response);    
        } catch (Exception ex) {
            System.out.println("Erro ao Carregar Alimento " + ex.getMessage());
            ex.printStackTrace();
        }
