 ...
 * @author Bruna Nobre
 */
@WebServlet(name = "AtivarPatologia", urlPatterns = {"/AtivarPatologia"})
public class AtivarPatologia extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (dao.inativar(patologia)) {
                mensagem = "Patologia ativado com sucesso!";
            } else {
                mensagem = "Problemas ao ativar Patologia!";
            }
            request.setAttribute("mensagem", mensagem);
            request.getRequestDispatcher("ListarPatologia").forward(request, response);
        } catch (Exception ex) {
            System.out.println("Problemas no Servlet ao ativar Patologia! Erro: " + ex.getMessage());
            ex.printStackTrace();
        }
    }
