 ...
 */
@WebServlet(name = "AtivarExercicio", urlPatterns = {"/AtivarExercicio"})
public class AtivarExercicio extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (dao.inativar(exercicio)) {
                mensagem = "Exercício ativado com sucesso!";
            } else {
                mensagem = "Problemas ao ativar Exercício!";
            }
            request.setAttribute("mensagem", mensagem);
            request.getRequestDispatcher("ListarExercicio").forward(request, response);
        } catch (Exception ex) {
            System.out.println("Problemas no Servlet ao ativar Exercício! Erro: " + ex.getMessage());
            ex.printStackTrace();
        }
